...
function leaderboard(name, n, generator, solutions) {
  ...
  for(let i = 0; i < solutions.length; i++) {
    ...
    const mean = Math.cbrt(size * tape * steps);
    successes.push({id: i + 1, solution, name, size, tape, steps, mean});
  }

  successes.sort(({mean: a}, {mean: b}) => a - b);
  ...
  console.log("|rank|id|entrant             |size|tape|   speed|");
  console.log("|----|--|--------------------|----|----|--------|");
  for(let i = 0; i < successes.length; i++) {
    const {id, name, size, tape, steps} = successes[i];
    console.log(
      "|%s|%s|%s|%s|%s|%s|",
